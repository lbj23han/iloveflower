...
### 2026-05-02 5~6월 꽃/축제 보강

단풍(`foliage`) 데이터 비중이 높아 5~6월 탐색 품질을 보강하기 위해 운영 Supabase에 장미, 수국, 라벤더, 작약, 등꽃, 능소화 스팟과 축제 데이터를 추가/갱신했습니다.

실행 스크립트:

```bash
node scripts/seed/seedMayJune2026Spots.cjs
...
- 능소화: `6 -> 8`
- 작약: `5 -> 8`
- 2026년 5~6월 축제 10건 연결 완료

검증 메모:

- Kakao Local API로 좌표를 재확인했습니다.
- `서울 창덕궁 등나무` 오매칭 행은 제거하고 `창덕궁 후원`에 등꽃 타입을 병합했습니다.
- `대구 달성 장미공원`은 Kakao 좌표 검색이 되지 않아 제외했습니다.
- 앱인토스 빌드는 `TOSS_BUILD` 환경에서 Supabase 브라우저 클라이언트로 같은 운영 DB를 조회합니다. 따라서 DB만 업데이트한 이번 변경은 별도 앱 배포 없이도 같은 DB를 바라보는 앱인토스 화면에 반영됩니다. 단, `/api/gyms` 경유 웹 빌드는 `s-maxage=60, stale-while-revalidate=300` 캐시가 있어 최대 몇 분 정도 지연될 수 있습니다.
