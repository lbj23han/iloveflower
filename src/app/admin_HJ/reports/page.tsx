      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/admin_HJ/posts" className="rounded-full border border-[#e5e7eb] bg-white px-3 py-1.5 text-xs font-medium text-[#6b7280] hover:border-[#ff6b81]">
              게시글 관리 →
            </Link>
            <button onClick={() => { localStorage.removeItem(ADMIN_SECRET_KEY); router.replace('/admin_HJ'); }} className="text-xs text-[#9ca3af] hover:text-red-400">
              로그아웃
            </button>
          </div>
        </div>
        ...
        <div className="mb-4 flex gap-2">
          {(['pending', 'approved', 'rejected'] as const).map((s) => (
            <button key={s} onClick={() => handleTabChange(s)}
              className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${tab === s ? 'bg-[#ff6b81] text-white' : 'border border-[#e5e7eb] bg-white text-[#6b7280]'}`}>
                ...
                <div key={r.id} className="rounded-2xl border border-[#e5e7eb] bg-white p-5 shadow-sm">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    <div>
                      ...
                      {r.flower_spots && (
                        <div className="text-xs text-[#6b7280]">기존 연결: {r.flower_spots.name} · {r.flower_spots.address}</div>
                      )}
                  ...
                  <div className="mb-3 flex flex-wrap gap-2 text-xs text-[#6b7280]">
                    ...
                    {r.has_night_light && <span>야간조명</span>}
                    {r.has_parking && <span>주차 가능</span>}
                    {r.pet_friendly && <span>반려동물 동반</span>}
                  </div>
                  {r.comment && (
                    <div className="mb-3 rounded-xl bg-[#f9fafb] px-3 py-2.5 text-sm text-[#374151]">&ldquo;{r.comment}&rdquo;</div>
                  )}
                    ...
                    <div className="mb-4 rounded-2xl border border-[#ffd6dc]/70 bg-[#fffafb] p-4">
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={query}
                          onChange={(e) => setMatchQueries((prev) => ({ ...prev, [r.id]: e.target.value }))}
                        ...
                        <button type="button" onClick={() => searchMatches(r.id, r.spot_name)}
                      ...
                      {selected && (
                        <div className="mt-3 flex items-start justify-between gap-3 rounded-xl bg-[#fff1f4] px-3 py-3">
                          <div>
                            ...
                            <div className="mt-1 text-sm text-[#374151]">{selected.name}{selected.address ? ` · ${selected.address}` : ''}</div>
                          </div>
                          <button type="button" onClick={() => setSelectedMatches((prev) => ({ ...prev, [r.id]: null }))}
                            className="rounded-full border border-[#ffd6dc] bg-white px-3 py-1 text-xs text-[#6b7280]">해제</button>
                        </div>
                      )}
                      {results.length > 0 && (
                        <div className="mt-3 space-y-2">
                          {results.map((spot) => (
                            <button key={spot.id} type="button"
                              onClick={() => setSelectedMatches((prev) => ({ ...prev, [r.id]: spot }))}
                              className="block w-full rounded-xl border border-[#e5e7eb] bg-white px-3 py-3 text-left hover:border-[#ff6b81]/40">
