...
import { createCommunityPost, listCommunityPosts } from '@/lib/appApi';
import {
  Post,
  POST_CATEGORIES,
...
export default function CommunityPanel() {
  ...
  useEffect(() => {
    ...
    listCommunityPosts(undefined, 20)
      .then(({ data }) => {
        if (!active) return;
        setPosts(data ?? []);
      })
  ...
  const submit = async () => {
    if (!title.trim() || !content.trim() || !postPassword.trim() || submitting) return;
    setSubmitting(true);
      ...
      {showForm && (
        <div className="space-y-3 border-b border-[#e5e7eb] bg-[#f9fafb] px-4 py-3 shrink-0">
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={submit}
              disabled={!title.trim() || !content.trim() || !postPassword.trim() || submitting}
        ...
        {loading ? (
          ...
          posts.map((post, index) => (
            <div key={post.id}>
              ...
              {(index + 1) % 5 === 0 && (
                <LazyTossAdBanner className="px-3 py-2" variant="card" />
              )}
            </div>
          ))
        )}
