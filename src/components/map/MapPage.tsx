...
function GlassPanel({
  ...
  return (
    <div
      className={`flex flex-col overflow-hidden rounded-[30px] border border-white/35 bg-white/58 shadow-[0_18px_50px_rgba(15,23,42,0.14)] backdrop-blur-xl ${className ?? ""}`}
    >
      {children}
    </div>
  );
    ...
    <button
      ...
      className={`flex items-center justify-center rounded-full transition-all ${
        ...
        active
          ...
          : "bg-white/72 text-[#6b7280] shadow-[0_8px_18px_rgba(15,23,42,0.10)]"
      }`}
      ...
      {top.length === 0 ? (
        ...
        top.map((gym, i) => (
          <div
            ...
            className="w-full flex items-center gap-2 px-3 py-2.5 hover:bg-white/50 text-left rounded-2xl transition-colors"
          >
    ...
    <div className="flex-1 overflow-y-auto py-1 px-1">
      {gyms.map((gym) => (
        <div
          ...
          onClick={() => onSelect(gym)}
          onKeyDown={(event) => handleKeyDown(event, gym)}
          className={`w-full flex items-center gap-2 rounded-2xl border px-3 py-2.5 text-left transition-colors hover:bg-white/50 ${
            selectedId === gym.id
              ? gym.photo_spot
      ...
      {recent.length === 0 ? (
        ...
        recent.map((gym, i) => (
          <button
            key={gym.id}
            onClick={() => onSelect(gym)}
            className="w-full flex items-start gap-2 px-3 py-2.5 hover:bg-white/50 text-left rounded-2xl transition-colors"
          >
...
export default function MapPage() {
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden">
      ...
      {zoomHint && (
        <div
          className="pointer-events-none absolute left-1/2 z-20 -translate-x-1/2 rounded-full border border-white/35 bg-white/70 px-4 py-2 text-sm text-[#4b5563] shadow-[0_10px_24px_rgba(15,23,42,0.12)] backdrop-blur-xl"
      ...
      {showMapSearchPrompt && boundsRef.current && (
        <div className="absolute left-1/2 top-1/2 z-20 -translate-x-1/2 -translate-y-1/2">
          <button
            ...
            className="rounded-full border border-white/50 bg-white/80 px-5 py-3 text-sm font-semibold text-[#111827] shadow-[0_16px_40px_rgba(15,23,42,0.14)] backdrop-blur-xl disabled:cursor-default disabled:opacity-90"
          >
            이 지역을 기반으로 검색하기
          </button>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-30 px-3 pt-3 md:px-4 md:pt-4">
        <div className="pointer-events-auto space-y-2">
          ...
          {/* 모바일 헤더: 한 줄 컴팩트 */}
          <div className="flex md:hidden items-center justify-between rounded-[22px] border border-white/45 bg-white/72 px-3 py-2 shadow-[0_16px_40px_rgba(15,23,42,0.12)] backdrop-blur-xl">
            <BrandLockup
              title="꽃놀이맵"
              iconSize={40}
              iconWidth={28}
          ...
          <div className="rounded-[24px] border border-white/40 bg-white/64 px-3 py-2 shadow-[0_12px_32px_rgba(15,23,42,0.10)] backdrop-blur-xl">
            <MarqueeTicker />
          </div>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex h-[46px] w-[46px] items-center justify-center rounded-full border shadow-[0_14px_36px_rgba(15,23,42,0.12)] backdrop-blur-xl transition-all ${
              searchOpen
                ...
                : "border-white/45 bg-white/72 text-[#111827]"
            }`}
            aria-label="시설 검색"
            title="시설 검색"
          >
          ...
          <button
            ...
            className="flex h-[46px] w-[46px] items-center justify-center rounded-full border border-white/45 bg-white/72 text-[18px] text-[#111827] shadow-[0_14px_36px_rgba(15,23,42,0.12)] backdrop-blur-xl transition-all hover:bg-white/84 disabled:cursor-default disabled:opacity-75"
            aria-label={
        ...
        {searchOpen && (
          <GlassPanel className="w-[min(360px,calc(100vw-2rem))] px-4 py-4">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-white/55 bg-white/78 px-4 py-3 text-sm text-[#111827] focus:border-[#111827] focus:outline-none"
            />

            <div className="mt-3 max-h-[320px] overflow-y-auto space-y-2">
              {searchLoading ? (
                <div className="rounded-2xl border border-white/45 bg-white/60 px-4 py-4 text-center text-sm text-[#6b7280]">
                  검색 중...
                </div>
              ) : searchQuery.trim().length < 2 ? (
                <div className="rounded-2xl border border-white/45 bg-white/60 px-4 py-4 text-center text-sm text-[#9ca3af]">
                  두 글자 이상 입력해 주세요
                </div>
                ...
                searchResults.map((gym) => (
                  <button
                    ...
                    className="w-full rounded-2xl border border-white/45 bg-white/68 px-4 py-3 text-left transition-colors hover:bg-white/82"
                  >
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              ...
              <button
                ...
                className="rounded-full border border-white/45 bg-white/72 px-3 py-1.5 text-xs font-medium text-[#4b5563]"
              >
                초기화
              </button>
            </div>
      ...
      >
        {leftPanelMode !== "closed" ? (
          ...
          <button
            ...
            className="self-start rounded-2xl border border-white/40 bg-white/66 px-3 py-4 text-sm text-[#6b7280] shadow-[0_14px_36px_rgba(15,23,42,0.14)] backdrop-blur-xl transition-colors hover:text-[#111827]"
          >
            ›
          </button>
        )}
      ...
      >
        {rightJudgeOpen ? (
          ...
          <button
            ...
            className="self-end rounded-2xl border border-white/40 bg-white/66 px-3 py-4 text-sm text-[#6b7280] shadow-[0_14px_36px_rgba(15,23,42,0.14)] backdrop-blur-xl transition-colors hover:text-[#111827]"
          >
            ‹
          </button>
        )}
      ...
      {mobileListOpen && (
        ...
        >
          <GlassPanel className="h-full">
            <div className="flex items-center justify-between border-b border-white/35 px-4 py-3">
      ...
      <button
        ...
        className="absolute bottom-[calc(86px+env(safe-area-inset-bottom))] left-3 z-20 rounded-full border border-white/45 bg-white/70 px-4 py-2 text-sm font-semibold text-[#111827] shadow-[0_14px_36px_rgba(15,23,42,0.14)] backdrop-blur-xl lg:hidden"
      >
      ...
      {selectedGym && (
        ...
        >
          <GlassPanel className="max-h-[84vh] bg-white/68">
      ...
      {supportOpen && (
        <div className="absolute inset-0 z-[60] flex items-center justify-center bg-[rgba(15,23,42,0.20)] px-4 backdrop-blur-[2px]">
          <div className="w-full max-w-md rounded-[28px] border border-white/50 bg-white/86 p-5 shadow-[0_24px_70px_rgba(15,23,42,0.18)] backdrop-blur-xl">
            <div className="mb-4 flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-full bg-white/72 text-sm text-[#6b7280] shadow-sm"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div className="rounded-[22px] border border-white/55 bg-white/76 px-4 py-3">
